...
def main(image, network='alexnet', size=227, layer='features.4', alpha=6, beta=2, 
    ...
    # [1,4,7,9,11]
    activations_sparsity,noisy_activations_sparsity,_,_ = setup_network_for_comprehensive_sparsity(model.features,layer_type=nn.Conv2d,noise_mag=1,layer_ixs=[1,4,7,9])    
...
#             acts = get_acts(model, x_)
        ...
        if False and 'jitter':
            ...
            masked = torch.roll(masked, shifts=(off1, off2), dims=(2, 3))            
        ...
        masked_gaussian_pyramid = gaussian_pyramid(masked, 7, 
        ...
        if method == 'fong':
            if LOSS_MODE=='max':
                ...
                tot_loss = alpha_lambda*alpha_term + 1e-2*1e-1*opts.tv_lambda*tv_term + opts.loss_lambda*loss_term + 1e-3* tv_conv_term
            # tot_loss = loss_term
