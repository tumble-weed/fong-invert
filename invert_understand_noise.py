...
def main(image, network='alexnet', size=227, layer='features.4', alpha=6, beta=2, 
    ...
    opts.SIMPLE_REF = False
    if opts.SIMPLE_REF:
        ref = -2.7*torch.ones_like(ref)
        # make a filled square in ref of square_size side length and centered on square_y,square_x
        square_size = 101
        ref[:,:,size//2-square_size//2:size//2+square_size//2,size//2-square_size//2:size//2+square_size//2] = 2.7
    ...
    opts.LOSS_MODE = 'match'
    opts.JITTER = False
    opts.noise_mag = 0.8
    opts.noise_mags = None
    opts.ZOOM_JITTER = False
    # opts.conv_layer_ixs = [11,9,7,4,1]
    opts.conv_layer_ixs = [9,7,4,1]
    if False:
        pass
    else:
    ...
    #===============================================
    if False and "replace relu in model with softplus":
        # import ipdb; ipdb.set_trace()
...
#         loss_f = lambda x: norm_loss(x, ref_acts)
    ...
    for i in range(opts.epochs):
        ...
        # import ipdb; ipdb.set_trace()
        # assert model.features.noise_mag == 0
        # if False and 'jitter':
        def log_grad(g):
            trends['loss_term_grad_mag'].append(g.abs().mean().item())
        x_for_forward.register_hook(log_grad)
        ...
        model.features.noise_flag = False
        with torch.no_grad():
            _ = model(x_for_forward)
        clean_acts = activations[0].detach().clone()
        model.features.noise_flag = original_noise_flag
        ...
        if opts.LOSS_MODE == 'match':
            # import ipdb; ipdb.set_trace()
            center_yx = acts.shape[2]//2, acts.shape[3]//2
            if True:
                with torch.no_grad():
                    clean_loss_term = norm_loss(clean_acts[...,center_yx[0],center_yx[1]], ref_acts[...,center_yx[0],center_yx[1]])
                loss_term = norm_loss(acts[...,center_yx[0],center_yx[1]], ref_acts[...,center_yx[0],center_yx[1]])

        else:
            LOSS_MODE = 'match'
            if LOSS_MODE == 'match':
                loss_term = norm_loss(acts, ref_acts)
                clean_loss_term = norm_loss(clean_acts, ref_acts)
        # """
        # if i == 150:
        # import ipdb; ipdb.set_trace()
        act_sparsity = (acts[0,:,center_yx[0],center_yx[1]] > 0).float().sum()
        ref_sparsity = (ref_acts[0,:,center_yx[0],center_yx[1]]>0).float().sum()
        # ref_sparsity,act_sparsity
        trends['ref_sparsity'].append(ref_sparsity.item())
        trends['act_sparsity'].append(act_sparsity.item())
        # """
        #===============================================================
        ...
        # import ipdb; ipdb.set_trace()
        #===============================================================
        # loss_term = -x_scores[:,opts.target_class].mean()
        ...
        if method == 'fong':
            ...
            if opts.LOSS_MODE == 'match':
                if model.features.noise_flag:
                    opts.tv_lambda = 0*1e-4
                else:
                    opts.tv_lambda = 0*1e-4
        ...
        trends['clean_loss_term'].append(np.log(clean_loss_term.item()+1))
        
        optimizer.zero_grad()
        
        network_internal_grad_per_location = torch.autograd.grad(acts.sum(),x_,retain_graph=True)
        network_internal_grad_per_location = network_internal_grad_per_location[0]
        tot_loss.backward(retain_graph=False)
        acts_mag_per_location = acts.sum(dim=1,keepdim=True).abs().mean()
        
        trends['acts_mag_per_location'].append(acts_mag_per_location.item())
        trends['network_internal_grad_per_location'].append(network_internal_grad_per_location.abs().mean().item())
        ...
        if (i+1) % opts.print_iter == 0:
            ...
            def save():
                ...
                if (len(opts.target_class) == 1) and (len(opts.image) == 1):                                  
                    ...
                    for lname in ['cam0_norm','cam0_sparsity','cam_norm','cam_tv','tv_term','loss_term','entropy','kl_from_start','kl_from_prev','change_in_mpool_from_prev','bg_entropy','x_entropy','grad_alignment','clean_loss_term','loss_term_grad_mag','acts_mag_per_location','network_internal_grad_per_location','ref_sparsity',
                                  'act_sparsity']:
...
def main2():
    ...
    if True:
        ...
        if False:
            ...
            args.size = 227
            args.layer = 'features.11'
            # args.layer = 'features.7'
