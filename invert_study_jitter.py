...
#             acts = get_acts(model, x_)
        ...
        if True and 'jitter':
            jitter = 30
            lim_0,lim_1 = jitter,jitter
            off1 = [random.randint(-lim_0, lim_0) for _ in range(n_noise)]
            off2 = [random.randint(-lim_1, lim_1) for _ in range(n_noise)]
            x_for_forward = torch.cat([torch.roll(x_for_forward[i:i+1], shifts=(off1[i], off2[i]), dims=(2, 3)) for i in range(n_noise)],dim=0)                    
        ZOOM_JITTER = False
        # MAX_SCALE=1.2
        MAX_SCALE=1.
        MIN_SCALE = 0.2
