...
def main(image, network='alexnet', size=227, layer='features.4', alpha=6, beta=2, 
    ...
    if method == 'fong':
        ...
        if False:
            ...
            n_noise = 100
            FEAT_VARIANCE_MODE = 'increase'
            if FEAT_VARIANCE_MODE == 'decrease':
                noise_sigma = 0.1        
            elif FEAT_VARIANCE_MODE == 'increase':
                noise_sigma = 0.01
            x__ = (torch.tensor( 
                prng.uniform(size = ref.size()) + noise_sigma * prng.randn(n_noise, *ref.size()[1:])
                )).float().to(device).clamp(0,1).requires_grad_(True)
...
#             acts = get_acts(model, x_)
        ...
        # import ipdb; ipdb.set_trace()
        mean_probs = x_probs.mean(dim=0,keepdim=True)
        # find kl divergence between the individual x_probs and the mean_probs
        clip_eps = 1e-7
        opts.T = 10
        x_probs_for_diversity = torch.softmax(x_scores/opts.T, dim=-1)
        mean_probs_for_diversity = x_probs_for_diversity.mean(dim=0,keepdim=True)
        x_probs_clipped = torch.clamp(x_probs_for_diversity,clip_eps,1-clip_eps)
        mean_probs_clipped = torch.clamp(mean_probs_for_diversity,clip_eps,1-clip_eps)
        eps = 0.0
        opts.JS = True
        if opts.JS:
            loss_diversity = F.kl_div(torch.log(x_probs_clipped + eps),
                                  mean_probs_clipped,reduction='batchmean') + F.kl_div(
                                  torch.log(mean_probs_clipped + eps),
                                  (x_probs_clipped),reduction='batchmean')
        else:
            loss_diversity = F.kl_div(torch.log(x_probs_clipped + eps),
                                  mean_probs_clipped,reduction='batchmean') 
            
        if 'misaligned grads':
            grad_for_diversity = torch.autograd.grad(loss_diversity, x_for_forward, retain_graph=True,create_graph=False)[0]
            def only_keep_misaligned(g):
                ...
                if True:
                    if False:
                        ...
                        FACTOR = 1
                        selected = (1 - torch.sign(grad_for_diversity) * torch.sign(g))/2. * (g) 
                ...
                # return torch.sign(g) * saliency 
                pass        
            x_for_forward.register_hook(only_keep_misaligned)
            
        # import ipdb; ipdb.set_trace()
        trends['loss_diversity'].append(loss_diversity.item())
        if method == 'fong':
            # score_sparsity_lambda = 0.009
            score_sparsity_lambda = 0.001*0
            loss_lambda = 1
            cam_weight = 0
            opts.diversity_lambda = 0
            ...
            tot_loss = alpha_lambda*alpha_term + tv_lambda*tv_term + loss_lambda*loss_term + score_sparsity_lambda*score_sparsity #- (opts.T)*opts.diversity_lambda*loss_diversity
        else:
            tot_loss = loss_term
        # tot_loss = loss_term
        #===============================================================
        ...
        if (i+1) % print_iter == 0:
            print('Epoch %d:\tAlpha: %f\tTV: %f\tLoss: %f\tTot Loss: %f\tCAM: %f\tprob/max_prob: %f/%f' % (i+1,
                ...
                cam.sum().item(),
                x_probs[:,target_class].mean().item(),x_probs.mean().max(dim=-1)[0].item()
                ))
            try:
                # utils.img_save2(x__,f'x_{i}.png')
                # utils.img_save2(x__,f'x.png')
                for j in [0,n_noise//4,2*n_noise//4,3*n_noise//4]:
                    utils.img_save2(x__[j:j+1],f'x_{j}_{i}.png')
                    utils.img_save2(x__[j:j+1],f'x_{j}.png')
            ...
            for lname in ['cam0_norm','cam0_sparsity','cam_norm','cam_tv','tv_term','loss_term','entropy','kl_from_start','kl_from_prev','change_in_mpool_from_prev','bg_entropy','x_entropy','loss_diversity']:
