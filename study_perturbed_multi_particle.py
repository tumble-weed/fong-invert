...
def main(image, network='alexnet', size=227, layer='features.4', alpha=6, beta=2, 
    ...
    if method == 'fong':
        ...
        if False:
            ...
            perturbation_mag = 1e-1 * 3e-1
            x__ = (torch.tensor( perturbation_mag * 
                prng.randn(*ref.size())
                )).float().to(device)
            
            x__ += Denormalize(utils.mu, utils.sigma)(ref)
            x__ = x__.clamp(0,1)
            x__ = x__.requires_grad_(True) 
...
#             acts = get_acts(model, x_)
        ...
        n_noise = 100
        noise_sigma = 0.1
        x_pre_noise_ = x_.clone()
        x_pre_noise_.retain_grad()
        x_ = x_pre_noise_ +  noise_sigma*torch.randn(n_noise,*x_.shape[1:],device=device)
        x_.retain_grad()
        #==================================================
        # median filter grads
        if False:
        ...
        feat_diversity = x_scores.std(dim=0).mean()
        #---------------------------------------------------------------
        loss_term = norm_loss(acts, ref_acts)
        # loss_term = -x_scores[:,target_class].mean()
        from interpolation_test import   interpolate
        mixed_x_ref_scores,mixed_x_ref = interpolate(x_,ref,10)
        mixed_acts = activations[0]
        interpolation_term = torch.stack([norm_loss(acts[None,:], ref_acts) for acts in mixed_acts])

        trends['interpolation_term'] = tensor_to_numpy(interpolation_term)
        
        if method == 'fong':
            if False:
                loss_lambda = 0.5
                print(colorful.yellow(f"multiplying loss term by factor {loss_lambda}"))
                tot_loss = alpha_lambda*alpha_term + tv_lambda*tv_term + loss_lambda*loss_term + 20*feat_diversity
            elif False:
                loss_lambda = 0.5
                print(colorful.yellow(f"multiplying loss term by factor {loss_lambda}"))
                tot_loss = alpha_lambda*alpha_term + tv_lambda*tv_term + loss_lambda*loss_term - 10*feat_diversity                
            elif True:
                loss_lambda = 0.5
                tv_lambda = 0
                print(colorful.yellow(f"multiplying loss term by factor {loss_lambda}"))
                tot_loss = alpha_lambda*alpha_term + tv_lambda*tv_term + loss_lambda*loss_term - 0*feat_diversity                                
        else:
            tot_loss = loss_term
        # tot_loss = loss_term
        trends['tv_term'].append(np.log(tv_term.item()+1))
        trends['loss_term'].append(np.log(loss_term.item()+1))
        trends['pixel_term'].append(np.log(pixel_term.item()+1))
        ...
        #---------------------------------------------------------------
        if 'inspect grads':
            x_grad = x_.grad*n_noise
            x_grad = (x_grad[:1] - x_grad[:1].min())/ (x_grad[:1].max() - x_grad[:1].min())
            utils.img_save2(x_grad,f'noisy_x_grad_{i}.png')
            x_pre_noise_grad = x_pre_noise_.grad
            x_pre_noise_grad = (x_pre_noise_grad - x_pre_noise_grad.min())/(x_pre_noise_grad.max() - x_pre_noise_grad.min())
            if noise_sigma > 0:
                utils.img_save2(x_pre_noise_grad,f'x_pre_noise_grad_{i}.png')
            roughness_x_grad = np.array([tv_norm(x_.grad[ii:ii+1]*n_noise)[0].item() for ii in range(n_noise)])
            roughness_x_pre_noise_grad = tv_norm(x_pre_noise_.grad)[0].item()
            print(roughness_x_grad.mean(),roughness_x_pre_noise_grad)
            
            x_grad = x_.grad*n_noise
            x_grad_std = x_grad.std(dim=0,keepdim=True)
            x_grad_std = x_grad_std.mean(dim=1,keepdim=True)
            x_grad_std = (x_grad_std - x_grad_std.min())/(x_grad_std.max() - x_grad_std.min())
            if noise_sigma > 0:
                utils.img_save2(x_grad_std,f'x_grad_std_{i}.png')
                utils.img_save2((x_grad_std > 0.5*x_grad_std.max()).float(),f'x_grad_std_{i}.png')
            if False:
                # import ipdb; ipdb.set_trace()
        ...
        if (i+1) % print_iter == 0:
            ...
            try:
                utils.img_save2(x__,f'multi_particle_x_{i}.png')
                utils.img_save2(x__,f'multi_particle_x.png')
            ...
            for lname in ['cam0_norm','cam0_sparsity','cam_norm','cam_tv','tv_term','loss_term','particle_feat_diversity','pixel_term']:
                utils.save_plot2(trends[lname],lname,f'{lname}.png')
            for lname in ['interpolation_term']:
                utils.save_plot2(trends[lname],lname,f'{lname}_{i}.png')
