...
#             acts = get_acts(model, x_)
        ...
        #---------------------------------------------------------------
        #====================================================================
        from my_gpdm import apply_swd
        swd_term = apply_swd(masked_pre_jitter,masks)
        trends['swd_term'].append(swd_term.item())
        #====================================================================
        LOSS_MODE='match'
        if LOSS_MODE=='match':
        ...
        opts.loss_lambda = 1*50 *0
        opts.tv_lambda = 1e-2*1e-1*1e-3*50*1e0 *0
        opts.tv_conv_lambda = 1*1e-3*1 * 1e-1 * 0
        if method == 'fong':
            if LOSS_MODE=='max':
                tot_loss = alpha_lambda*alpha_term + tv_lambda*tv_term + 1e-2*loss_term
            elif LOSS_MODE=='match':
                tot_loss = alpha_lambda*alpha_term + opts.tv_lambda*tv_term + opts.loss_lambda*loss_term + opts.tv_conv_lambda* tv_conv_term + 1e0*swd_term
            # tot_loss = loss_term
        ...
        if (i+1) % print_iter == 0:
            ...
            for lname in ['tv_term','loss_term','tv_conv_term','mean_grad','swd_term']:
