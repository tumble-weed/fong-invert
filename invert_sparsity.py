...
def main(image, network='alexnet', size=227, layer='features.4', alpha=6, beta=2, 
    ...
    activations_sparsity,noisy_activations_sparsity = setup_network_for_comprehensive_sparsity(model.features,layer_type=nn.ReLU,noise_mag=0.)
...
#             acts = get_acts(model, x_)
        ...
        assert activations_last_mpool_.min() >= 0
        ...
        if False:
            ...
            sparsity_lambdas[0] = 0.001 
            sparsity_lambdas[2] = 0.005 * float(i< 200) + 0.001 * float(i> 200)
        ...
        score_sparsity = x_scores.norm(2)
        if method == 'fong':
            # score_sparsity_lambda = 0.009
            score_sparsity_lambda = 0.01 * 1
            loss_lambda = 1
            cam_weight = 0
        ...
        if 'cam loss':
            if True:
                ...
                if i > 0:
                    cam_loss = cam_weight*cam_norm + 10*tv_cam + 0*bg_entropy
            else:
