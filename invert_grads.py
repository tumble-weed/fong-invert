...
def main(image, network='alexnet', size=227, layer='features.4', alpha=6, beta=2, 
    ...
    if True:
        ...
        ref_grads = get_pytorch_module(model, layer_name).grads.detach().clone()
        # ref_cam_weights = ref_grads.mean(dim=(-1,-2)).squeeze(0)
    # assert False
...
#         loss_f = lambda x: norm_loss(x, ref_acts)
    ...
    for i in range(opts.epochs):
        ...
        x_scores[:,opts.target_class[0]].sum().backward(retain_graph=True)
        x_grads = get_pytorch_module(model, layer_name).grads
        ...
        #---------------------------------------------------------------        
        # assert False
        if opts.LOSS_MODE == 'match_grads':
            loss_term = ((ref_grads - x_grads)**2).sum()
