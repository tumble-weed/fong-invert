...
def main(image, network='alexnet', size=227, layer='features.4', alpha=6, beta=2, 
    ...
    if method == 'fong':
        ...
        optimizer_saliency = torch.optim.SGD([mask_logit_full], lr=learning_rate, momentum=momentum)
    elif method == 'dip':
        ############################################################
        imsize = 224
...
#             acts = get_acts(model, x_)
        ...
        # import ipdb; ipdb.set_trace()      
        n_areas = 10
        smoothen = GaussianSmoothing(1, (window_size,window_size),     
        (window_size - 1)/4, 
        dim=2,device=device)
        max_filter = SoftMaxFilter2d((window_size,window_size),1,(1,1),
                                        padder =  'reflection',
            # sharpness=1e-1
            sharpness=1e1
            ).to(device)
        ...
        # max_of_median_conv_mask_full = torch.cat([ get_max_of_median_mask(
        #             m.unsqueeze(0),
        #             median_filter_for_forward,
        #             max_after_median_for_forward) for mi,m in enumerate(mask_logit_clones) ],dim=0)
        from areas import get_thresholds
        # assert False            
        
        # required_areas =  torch.linspace(
        #     OPTIONS['thresholding']['rel_rank_thresh_min'] +
        #     (OPTIONS['thresholding']['rel_rank_thresh_max']/K)*float((e+1)%K),
        #     OPTIONS['thresholding']['rel_rank_thresh_max']/K +
        #     (OPTIONS['thresholding']['rel_rank_thresh_max']/K)*float((e+1)%K),
        #     n_thresh//OPTIONS['thresholding']['n_repeat_per_thresh']).to(device)*mask_.shape[0]
        sharpness = 90
        area_l = 2
        required_areas=torch.linspace(0,1*np.prod(max_of_smooth_mask.shape[-2:]),n_areas).to(device)
        thresholds = get_thresholds(max_of_smooth_mask,sharpness = sharpness,
                        required_areas=required_areas,l=area_l)
        

        multi_mask = multi_thresh_sigmoid(max_of_smooth_mask,thresholds,sharpness,trends)
        ...
        masked,_ = get_masked_input(
                    ...
                    smooth=0)            
        '''
        masked = laplacian_pyramid_blending(x_, ref, multi_mask, num_levels=6)

        # show_scores = masked_forward(model,ref,mask_multi_ste,masking_options=OPTIONS
        #=============================================
        x_scores = model(masked)
        #utils.img_save2(masked[1:2],f'masked_{i}.png')
        #utils.img_save2(multi_mask[1:2],f'masked_{i}.png')
        import ipdb; ipdb.set_trace()
        ...
        if (i+1) % print_iter == 0:
            ...
            for lname in ['cam0_norm','cam0_sparsity','cam_norm','cam_tv','tv_term','loss_term','entropy','kl_from_start','kl_from_prev','change_in_mpool_from_prev']:
