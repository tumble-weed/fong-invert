...
def main(image, network='alexnet', size=227, layer='features.4', alpha=6, beta=2, 
    ...
    if method == 'fong':
        ...
        if False:
            ...
            x_mask = (torch.tensor( 
                prng.uniform(size = (n_unique_x,1)+ref.shape[2:])
                )).float().to(device).requires_grad_(True)
            fg_size = 71
        if opts.use_mask:    
            optimizer = torch.optim.SGD([x_fg,x_bg,x_mask], lr=opts.learning_rate, momentum=momentum)
        else:
            optimizer = torch.optim.SGD([x_fg,x_bg], lr=opts.learning_rate, momentum=momentum)
...
#             acts = get_acts(model, x_)
        ...
        if False and method == 'dip':
            ...
            fg_mask = torch.zeros_like(x_fg)
            fg_mask[:,:,(fg_mask.shape[2] - fg_size)//2:- ( fg_mask.shape[2] -fg_size)//2, (fg_mask.shape[3] -fg_size)//2:-(fg_mask.shape[3] -fg_size)//2] = 1
            n_translate = 10
            max_translate = fg_size//2
            # ty,tx = random.randint(-max_translate, max_translate),random.randint(-max_translate, max_translate)
            ty = [random.randint(-max_translate, max_translate) for _ in range(n_translate)]
            tx = [random.randint(-max_translate, max_translate) for _ in range(n_translate)]
            
            fg_rolled = torch.cat([torch.roll(x_fg, shifts=(ty_, tx_), dims=(2, 3)) for ty_,tx_ in zip(ty,tx)])
            mask_rolled = torch.cat([torch.roll(fg_mask, shifts=(ty_, tx_), dims=(2, 3)) for ty_,tx_ in zip(ty,tx)])
            x_mask_rolled = torch.cat([torch.roll(x_mask, shifts=(ty_, tx_), dims=(2, 3)) for ty_,tx_ in zip(ty,tx)])
            # randomly horizontal flip the fg
            flip_flags = torch.randint(0,2,fg_rolled.shape[:1],device=device)
            def hflip(tensor,flip_flags):
                tensor = tensor * (1 - flip_flags.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)) + torch.flip(tensor, dims=[-1]) * flip_flags.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)
                return tensor
            fg_rolled = hflip(fg_rolled,flip_flags)
            mask_rolled = hflip(mask_rolled,flip_flags)
            x_mask_rolled  = hflip(x_mask_rolled,flip_flags)
            
            # utils.cipdb('DBG_ALIGN')
            x_bg_for_full = x_bg.clone()
            x_bg_for_full.retain_grad()
            if opts.use_mask:
                
                x_bg_fg = ((
                        mask_rolled * x_mask_rolled  * fg_rolled 
                        + mask_rolled * (1 - x_mask_rolled)  * x_bg_for_full
                        + (1 - mask_rolled)  *x_bg_for_full))
                x_bg_fg_for_tv = ((
                        mask_rolled * x_mask_rolled  * fg_rolled 
                        + mask_rolled * (1 - x_mask_rolled)  * x_bg
                        + (1 - mask_rolled)  *x_bg))

            else:
                x_bg_fg = ((mask_rolled * fg_rolled + (1 - mask_rolled) * x_bg_for_full))
                x_bg_fg_for_tv = ((mask_rolled * fg_rolled + (1 - mask_rolled) * x_bg))
            # x_bg_fg.retain_grad()
            x_ = (x_bg_fg  - 0.5)*2*3
            # utils.cipdb('DBG_ALIGN')
            x_bg_for_bg = x_bg.clone()
            x_bg_for_bg.retain_grad()
            x_bg_ = (x_bg_for_bg - 0.5)*2*3
            # import ipdb; ipdb.set_trace()
        ...
        tv_norm_x,tv_map = tv_norm(x_bg_fg_for_tv, beta=beta)
        tv_norm_trunc_x,tv_trunc_map = tv_norm_trunc(x_bg_fg_for_tv, beta=beta, T = 0.15)
        if x_bg_fg_for_tv.shape[0] == 1:
        ...
        if False and opts.LOSS_MODE == 'match':
            ...
            # import ipdb; ipdb.set_trace()
            loss_term_bg_fg = -x_scores[:,opts.target_class[0]].mean(dim=0)
            loss_term_bg = bg_scores[:,opts.target_class].sum()
            loss_term = opts.bg_weight*loss_term_bg + opts.fg_weight*loss_term_bg_fg
            # if opts.aligned_backprop:
            # utils.cipdb('DBG_ALIGN')
            bg_grad = torch.autograd.grad(loss_term_bg, x_bg_for_bg, retain_graph=True)[0]
            from aligned_backprop import register_keep_according_to_alignment
            register_keep_according_to_alignment(x_bg_for_full,x_bg_for_bg,bg_grad,mode="aligned",FACTOR=opts.bg_weight)                
        #===============================================================
        ...
        if method == 'fong':
            if any([
                x_bg.grad.isnan().any(),x_bg.grad.isinf().any(),
                x_fg.grad.isnan().any(),x_fg.grad.isinf().any()
            ]):
        ...
        if 'gradcam':
            ...
            try:
                ...
                weights = torch.zeros_like(gradcam_acts)
        ...
        if opts.simonyan_saliency:
            ...
            saliency = torch.zeros_like(x_fg)
        ...
        if (i) % opts.print_iter == 0:
            ...
            def save():
                ...
                try:
                    if (len(opts.target_class) == 1) and (len(opts.image) == 1):
                        utils.img_save2(x_bg_fg,f'x_iter_{i}.png')
                        utils.img_save2(x_bg_fg,f'x.png',syncable=True)
                        utils.img_save2(x_mask,f'x_mask.png',syncable=True)
                        utils.img_save2(x_bg,f'x_bg.png',syncable=True)
                        utils.img_save2(x_fg,f'x_fg.png',syncable=True)
                    for ii,target_class in enumerate(opts.target_class):
                        utils.img_save2(x_bg_fg[ii:ii+1],f'x_{target_class}_iter_{i}.png')
                        utils.img_save2(x_bg_fg[ii:ii+1],f'x_{target_class}.png',syncable=True) 
                    for ii in range(ref.shape[0]):
                        # get imroot from opts.image[ii]
                        imroot = os.path.basename(opts.image[ii]).split('.')[0]
                        utils.img_save2(x_bg_fg[ii:ii+1],f'x_{imroot}_iter_{i}.png')
                        utils.img_save2(x_bg_fg,f'x_{imroot}.png',syncable=True)                        
                ...
                with open(os.path.join(utils.SAVE_DIR,'trends.pkl'),'wb') as f:
                    ...
                    pickle.dump(tensor_to_numpy(x_bg),f)
                    pickle.dump(tensor_to_numpy(x_fg),f)
                    pickle.dump(tensor_to_numpy(x_mask),f)
                ...
                SAVE_DONE[0] = True
            if any([
                os.environ.get('DBG_SAVE',False),
                os.environ.get('DBG_MASK',False)
            ]):  
                save()    
            else:
        ...
        if True and method == 'fong':
            x_fg.data.copy_(torch.clamp(x_fg, 0, 1))
            x_bg.data.copy_(torch.clamp(x_bg, 0, 1))
            x_mask.data.copy_(torch.clamp(x_mask, 0, 1))            
