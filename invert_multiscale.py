...
#             acts = get_acts(model, x_)
        ...
        if True:
            ...
            if i < 100:
                SCALE_FACTOR = 16
                optimizer.param_groups[0]['lr'] = 1e-1 * 16
            if i > 100 and i < 200:
                SCALE_FACTOR = 8
                optimizer.param_groups[0]['lr'] = 1e-1 * 8
            # if i > 200 and i < 300:
            #     SCALE_FACTOR = 4
            """        
            # MAX_SCALE = 12
            ...
            MAX_SCALE = 32
            MAX_ITER = 500
            SCALE_FACTOR_FLOAT = max(MAX_SCALE * (MAX_ITER-i)/MAX_ITER,1)
            print(colorful.red(f"SCALE:{SCALE_FACTOR_FLOAT}"))
            # SCALE_FACTOR = max(int(SCALE_FACTOR_FLOAT),1)
            # print(colorful.red(f"SCALE:{SCALE_FACTOR}"))
            x_pre_blur = x_
            new_shape_coarser = ( int(ref.shape[-2]/SCALE_FACTOR_FLOAT),int(ref.shape[-1]/SCALE_FACTOR_FLOAT))
            x_coarser_ = torch.nn.functional.interpolate(x_, size = new_shape_coarser, mode='bilinear', align_corners=False,antialias=True)
            x_coarser_ = torch.nn.functional.interpolate(x_coarser_, size = ref.shape[-2:], mode='bilinear', align_corners=False,antialias=True)
            if False:
                x_finer_ = torch.nn.functional.interpolate(x_, size = (ref.shape[-2]//SCALE_FACTOR,ref.shape[-1]//SCALE_FACTOR), mode='bilinear', align_corners=False,antialias=True)
                x_finer_ = torch.nn.functional.interpolate(x_finer_, size = ref.shape[-2:], mode='bilinear', align_corners=False,antialias=True)            
            x_ = x_coarser_
            # optimizer.param_groups[0]['lr'] = max(SCALE_FACTOR//2,1)*1e-1  #* 16/(15 * (i/300) + 1)
            x_ = (x_pre_blur - x_).detach()  + x_
            # x_ = x_pre_blur + (x_ - x_pre_blur).detach()
        if False:
            start_scale = 1
            scale = (start_scale + (1 - start_scale)*i/300)
            print(colorful.red(f"SCALE:{scale}"))
            x_,_ =  get_masked_input(
                                        x_,
                                        # 1-torch.sigmoid( 100 * (cam_nrmz_up - 0.5)) ,
                                        torch.ones(1,1,*x_.shape[-2:],device=device).float()*scale,
                                        # perturbation=BLUR_PERTURBATION,
                                        num_levels=8,
                                        # variant=DELETE_VARIANT,
                                        smooth=0)
            optimizer.param_groups[0]['lr'] = 1*1e-1  #* 16/(15 * (i/300) + 1)
