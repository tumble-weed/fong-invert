...
def main(image, network='alexnet', size=227, layer='features.4', alpha=6, beta=2, 
    ...
    assert len(activations) == 1
    ref_acts = activations[0].detach().clone()
    # import ipdb; ipdb.set_trace()
    if True and method == 'fong':
        ...
        if False:
            ...
            # perturbation_mag = (2*5e-1) * 3e-1
            perturbation_mag = 1* 3e-1
            x__ = (torch.tensor( perturbation_mag * 
                prng.randn(*ref.size())
                )).float().to(device)
            
            x__ += Denormalize(utils.mu, utils.sigma)(ref.detach().clone())
            x__ = x__.clamp(0,1)
...
#             acts = get_acts(model, x_)
        ...
        tv_norm_trunc_x,tv_trunc_map = tv_norm_trunc(x__, beta=beta, T = 0.9,mode='penalize-larger')
        ...
        # import ipdb; ipdb.set_trace()
        # loss_term = ((ref - x_)**2).sum() * 1e-3
        if 'interpolation':
            n_interp = 50
            mixed_x_ref_scores,mixed_x_ref = interpolate(x_,ref,model,n_interp)
            mixed_acts = activations[0]
            assert mixed_acts.shape[0] == n_interp
            # interpolation_term = [norm_loss(acts[None,:], ref_acts) for acts in mixed_acts]
        ...
        if (i+1) % print_iter == 0:
            ...
            for lname in ['cam0_norm','cam0_sparsity','cam_norm','cam_tv','tv_term','loss_term','entropy','kl_from_start','kl_from_prev','change_in_mpool_from_prev','pixel_term']:
                utils.save_plot2(trends[lname],lname,f'{lname}.png')
            for lname in ['tv_map']:
                utils.save_histogram2(tensor_to_numpy(locals()[lname]).flatten(),lname+'_hist',f'{lname}_hist_{i}.png')
