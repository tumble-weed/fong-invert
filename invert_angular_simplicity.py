...
def main(image, network='alexnet', size=227, layer='features.4', alpha=6, beta=2, 
    ...
    activations_sparsity,noisy_activations_sparsity,avg_angles,avg_mags = setup_network_for_comprehensive_sparsity(model.features,layer_type=nn.Conv2d,noise_mag=0)
...
#             acts = get_acts(model, x_)
        ...
        if True:
            ...
            # angular_lambdas = [0.0005 for _ in sparsities]
            opts.angular_lambdas = [0 for _ in sparsities]
            if True:
                opts.angular_lambdas[0] = 1000/2
                opts.angular_lambdas[1] = 1000/2
                opts.angular_lambdas[2] = 1000/2
                opts.angular_lambdas[3] = 1000/2
                opts.angular_lambdas[4] = 1000/2
            elif False and i > 100:
                angular_lambdas[0] = 0.001
                # angular_lambdas[1] = 0.001
                # angular_lambdas[2] = 0.005
                # angular_lambdas[3] = 0.01
            elif False:
                angular_lambdas[0] = 0.001 
                angular_lambdas[2] = 0.005 * float(i< 200) + 0.001 * float(i> 200)
            # import ipdb; ipdb.set_trace()
            angular_simplicity = sum([opts.angular_lambdas[i]*avg_angles[i][0] for i in range(len(sparsities))])
        else:
            angular_simplicity = 0            
        ...
        if method == 'fong':
            ...
            opts.ANGULAR_SIMPLICITY_MODE = 'decrease'
            score_sparsity_lambda = 0.001 * 0.1 * 0
            opts.loss_lambda = 1
            cam_weight = 5*0
            last_mpool_sparsity_lambda = 0*0.001
            opts.tv_lambda = 1e-1
            opts.angular_simplicity_lambda = 1
            ...
            print(colorful.orange(f"using loss_lambda = {opts.loss_lambda}"))
            if False and 'last_mpool_sparsity':
                ...
                # assert opts.ANGULAR_SIMPLICITY_MODE == 'increase'
                tot_loss = alpha_lambda*alpha_term + opts.tv_lambda*tv_term + opts.loss_lambda*loss_term + score_sparsity_lambda*score_sparsity + opts.angular_simplicity_lambda*angular_simplicity * (1 if opts.ANGULAR_SIMPLICITY_MODE == 'decrease' else  -1)
        else:
            tot_loss = loss_term
        # tot_loss = loss_term
        #===============================================================
        ...
        if 'cam loss':
            if True:
                ...
                if i > 0:
                    cam_loss = cam_weight*cam_norm + 0*tv_cam + 0*bg_entropy.sum()
            else:
