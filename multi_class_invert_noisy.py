...
def main(image, network='alexnet', size=227, layer='features.4', alpha=6, beta=2, 
    ...
    utils.set_save_dir(utils.DEBUG_DIR,MAJOR_PREFIX,MINOR_PREFIX,purge=opts.purge)
    # import ipdb; ipdb.set_trace()
    ...
    if False:
        ...
        if True:
            activations = []
            setup_network(model,get_pytorch_module(model, layer),activations)
            # setup for gradcam
    ...
    if os.environ.get('EDGE_ALIGN',False):
    ...
    if method == 'fong':
        ...
        if False:
            ...
            x__ = (torch.tensor( 
                prng.uniform(size = (len(opts.target_class),)+ref.shape[1:])
                )).float().to(device).requires_grad_(True)
        # import ipdb;ipdb.set_trace()
        # utils.cipdb('DBG_MULTI')
...
#             acts = get_acts(model, x_)
        ...
        opts.ZOOM_JITTER = opts.settings['ZOOM_JITTER']
        opts.MAX_SCALE=1.2
        ...
        if opts.LOSS_MODE == 'match':
            assert False,'copy the match loss from multi_image_invert_noisy'
            ...
            loss_term = -x_scores[:,target_classes].mean()
        #===============================================================
            ...
            def save():
                ...
                try:
                    ...
                    for ii,target_class in enumerate(opts.target_class):
                        ...
                        utils.img_save2(x__[ii:ii+1],f'x_{target_class}.png')
