...
#             acts = get_acts(model, x_)
        ...
        #*****************************************************************
        # nrmz_x__ = (x__ - x__.min())/(x__.max() - x__.min())
        # gray_x__ = x__.mean(dim=1,keepdim=True).tile(1,3,1,1)
        tv_norm_x,tv_map = tv_norm(
                                    x__, 
                                    # gray_x__,
                                   beta=beta)
        
        
        tv_norm_trunc_x,tv_trunc_map = tv_norm_trunc(
                                                    x__,
                                                    # nrmz_x__, 
                                                    # gray_x__,
                                                     beta=beta, 
        ...
        if method == 'fong':
            ...
            tot_loss = alpha_lambda*alpha_term + tv_lambda*tv_term + loss_lambda*loss_term + (1 if (FEAT_VARIANCE_MODE == 'decrease') else -1 ) * diversity_lambda*feat_diversity + score_sparsity_lambda*score_sparsity
        ...
        if (i+1) % print_iter == 0:
            ...
            try:
                for j in [0,n_noise//4,2*n_noise//4,3*n_noise//4]:
                    ...
                    utils.img_save2(x__[j:j+1],f'multi_particle_x_{j}.png')
