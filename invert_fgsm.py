...
from adversarial_attack import fgsm_attack
torch.manual_seed(0)
...
#             acts = get_acts(model, x_)
        ...
        if method == 'dip':
            ...
            x__model_ = x__.detach().clone().requires_grad_(True)
            # x__model_.retain_grad()
            x_ = (x__model_- 0.5)*2
        ...
        trends['x_probs'].append(x_probs[:,target_class].mean().item())
        optimizer.zero_grad()
        tot_loss.backward(retain_graph=True)
        #=================================================
        # if x_scores.max(1)[1].item() == target_class:
        #     fgsm_flag = False
        # else:
        #     fgsm_flag = True
        if fgsm_flag:
            if x_scores.max(1)[1].item() == target_class:
                ...
                # import ipdb; ipdb.set_trace()
                print(x_scores.max(1)[1].item())
                epsilon = 0.25
                perturbed_x__ = fgsm_attack(x__, epsilon, -x__model_.grad)
                x__.data.copy_(perturbed_x__)
                # x__.grad.data.copy_(torch.zeros_like(x__.grad))
        else:
            x__.grad.data.copy_(x__.grad + x__model_.grad)
        #=================================================
        if method == 'fong':
        ...
        if (i+1) % print_iter == 0:
            ...
            for lname in ['cam0_norm','cam0_sparsity','cam_norm','cam_tv','tv_term','loss_term','x_probs']:
